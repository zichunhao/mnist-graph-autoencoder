...
def train(args, model, loader, epoch, optimizer, outpath, is_train):
    ...
    save_gen_imgs(args, gen_imgs, labels, epoch, is_train, outpath)
...
def train_loop(args, model, train_loader, valid_loader, optimizer, outpath):
    ...
    for ep in range(args.num_epochs):
        ...
        save_data(args, data=train_avg_loss, data_name="loss", epoch=epoch, outpath=outpath, is_train=True)
        save_data(args, data=train_dt, data_name="dt", epoch=epoch, outpath=outpath, is_train=True)

        # Validation
        start = time.time()
        valid_avg_loss, _ = train(args, model, valid_loader, epoch, optimizer, outpath, is_train=False)
        valid_dt = time.time() - start

        valid_avg_losses.append(train_avg_loss)
        valid_dts.append(valid_dt)

        save_data(args, data=valid_avg_loss, data_name="loss", epoch=epoch, outpath=outpath, is_train=False)
        save_data(args, data=valid_dt, data_name="dt", epoch=epoch, outpath=outpath, is_train=False)
    ...
    # Save global data
    save_data(args, data=train_avg_losses, data_name="losses", epoch="global", outpath=outpath, is_train=True, global_data=True)
    save_data(args, data=train_dts, data_name="dts", epoch="global", outpath=outpath, is_train=True, global_data=True)
    save_data(args, data=valid_avg_losses, data_name="losses", epoch="global", outpath=outpath, is_train=False, global_data=True)
    save_data(args, data=valid_dts, data_name="dts", epoch="global", outpath=outpath, is_train=False, global_data=True)
